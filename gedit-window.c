...
gedit_window_class_init (GeditWindowClass *klass)
{
	...
	gtk_widget_class_bind_template_child_private (widget_class, GeditWindow, language_button);
        // NV
        gtk_widget_class_bind_template_child_private (widget_class, GeditWindow, word_suggestions_button);
        // NV
                      ...
                      GeditWindow                *window)
{
	...
	/* splchk_set_lang() declared and Defined in splchk_functions.h and splchk_functions.c */
        
        /* gtk_source_language_get_name() returns language->priv->name */
        
        if(language != NULL)
                splchk_set_lang((char *)gtk_source_language_get_name(language));
        else
                splchk_set_lang("Plain Text");
...
/* gedit_word_selector_build_list() declared and defined in gedit-word-selector.h and gedit-word-selector.c */

static gboolean on_popover_shown (GtkWidget *widget, GdkEventButton *event, gpointer *window) {
    gedit_word_selector_build_list (((GeditWindow *)window)->priv->selector);
    gtk_widget_show_all (((GeditWindow *)window)->priv->word_suggestions_popover);
    return TRUE;
}

static gboolean on_shortcut_pressed_popover_shown (GtkWidget *widget, GdkEventKey *event, gpointer *window) {
...
setup_statusbar (GeditWindow *window)
{
	...
	gtk_widget_show (GTK_WIDGET (sel));
        ...
        gedit_status_menu_button_set_label (GEDIT_STATUS_MENU_BUTTON (window->priv->word_suggestions_button), ("Word Suggestions"));
        
        window->priv->word_suggestions_popover = gtk_popover_new (window->priv->word_suggestions_button);
        gtk_menu_button_set_popover (GTK_MENU_BUTTON (window->priv->word_suggestions_button),
                                     window->priv->word_suggestions_popover);
        
        window->priv->selector = gedit_word_selector_new ();
        
        g_signal_connect (window->priv->word_suggestions_button, "button_press_event", G_CALLBACK (on_popover_shown), window);
        
        g_signal_connect_after (window, "key-press-event", G_CALLBACK (on_shortcut_pressed_popover_shown), window);
        
        gtk_container_add (GTK_CONTAINER (window->priv->word_suggestions_popover), GTK_WIDGET(window->priv->selector));
        
        gtk_widget_show (GTK_WIDGET(window->priv->selector));
        
        /*  splchk_set_lang() and splchk_get_words() and Defined in splchk_functions.h and splchk_functions.c */
		  ...
		  GeditView   *new_view)
{
	...
	if (new_view)
	{
		...
		gtk_widget_show (window->priv->language_button);
                gtk_widget_show (window->priv->word_suggestions_button);
		...
		GeditWindow        *window)
{
	...
	if (tab == gedit_multi_notebook_get_active_tab (multi))
	{
		...
		// NV
		if (window->priv->word_suggestions_id)
		{
			g_signal_handler_disconnect (doc, window->priv->word_suggestions_id);
			window->priv->word_suggestions_id = 0;
		}
		// NV
	...
	if (num_tabs == 0)
	{
		...
		gtk_widget_hide (window->priv->language_button);
                gtk_widget_hide (window->priv->word_suggestions_button);
	}
